...
public class BuildApp {
    ...
    public static void Share() {
        try {
            ...
            intent.putExtra(Intent.EXTRA_TEXT, "اپلیکیشن " + LuancherActivity.activity.getResources().getString(R.string.app_name) + " رو از " + BuildApp.siteName + " دانلود کن:\n" + BuildApp.download);
            intent.setType("text/*");
            intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            LuancherActivity.activity.startActivity(Intent.createChooser(intent, "برنامه ای را جهت ارسال انتخاب نمائید"));
        } catch (Exception e) {
            Log.e("rr", e.toString());
        }
    }
